...
if  selected == "Home":
    ...
    with st.container():
        ...
        with st.container():
            ...
            st.markdown("<h2 style='text-align: center; color: orange;'>Book a Taxi in your city</h2>", unsafe_allow_html=True)
            st.markdown("<h6 style='text-align: center; color: white;'>choose from a range of categories and prices</h6>", unsafe_allow_html=True)
...
if selected == "Faqs":
    ...
    if faq == "diff-car-types":
        ...
        query = "select car_type from cars"
        cur = con.cursor()
        cur.execute(query)
        data = cur.fetchall()
        st.dataframe(data)
